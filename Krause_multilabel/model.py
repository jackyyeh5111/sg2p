...
class Regions_Hierarchical():
    def __init__(self, word2idx,
            ...
            self.label_W = tf.Variable(tf.random_uniform([self.sentRNN_lstm_dim, self.label_size], -0.1, 0.1), name='label_W')
            ...
            # self.random_uniform_initializer = tf.random_uniform_initializer(-0.1, 0.1)
            # self.label_layer_1 = Dense(self.project_dim, kernel_initializer=self.random_uniform_initializer, activation=tf.nn.relu, name="label_layer_1")
            # self.label_layer_2 = Dense(self.label_size, kernel_initializer=self.random_uniform_initializer, activation=tf.nn.relu, name="label_layer_2")

            
            self.weight_initializer = tf.contrib.layers.xavier_initializer()
            self.const_initializer = tf.constant_initializer(0.0)

    def _get_label_vec(self, h, reuse=None):
        
        label_vec = tf.matmul(h, self.label_W) + self.label_b 
        
        # maxpooling_vec = tf.reduce_max(features_proj, reduction_indices=1)
        # _input = tf.concat([maxpooling_vec, h], 1)

        # hidden = self.label_layer_1(_input)
        # label_vec = self.label_layer_2(hidden)

        return label_vec
            
    def _get_initial_lstm(self, features, reuse=None):
        with tf.variable_scope('initial_lstm', reuse=reuse):
            features_mean = tf.reduce_max(features, 1)
    ...
    def _attention_layer(self, features, features_proj, label_vec, reuse=False):
        with tf.variable_scope('attention_layer', reuse=reuse):
            w = tf.get_variable('w', [self.label_size, self.D], initializer=self.weight_initializer)
            b = tf.get_variable('b', [self.D], initializer=self.const_initializer)
            w_att = tf.get_variable('w_att', [self.D, 1], initializer=self.weight_initializer)

            label_att = tf.nn.relu(features_proj + tf.expand_dims(tf.matmul(label_vec, w), 1) + b)    # (N, L, D)
            out_att = tf.reshape(tf.matmul(tf.reshape(label_att, [-1, self.D]), w_att), [-1, self.L])   # (N, L)
    ...
    def build_model(self, S_max, semi=False, reuse=False):
        ...
        # for multilabel
        label_vec = self._get_label_vec(h)
        ...
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            for i in range(0, S_max):
                ...
                cross_entropy = tf.losses.sigmoid_cross_entropy(
                    logits= label_vec, multi_class_labels=self.caption_labels[:, i, :])
                cross_entropy = cross_entropy * sents_mask[:, i]
                loss_label = tf.reduce_mean(cross_entropy)

                loss += loss_label * lambda_label

                    
                context, alpha = self._attention_layer(features, features_proj, label_vec, reuse=reuse or (i!=0))
                ...
                label_vec = self._get_label_vec(word_h)
    ...
    def build_sampler(self, reuse):
        ...
        for i in range(0, self.S_max):
            ...
            label_vec = tf.nn.softmax(label_vec)
            total_pred_labels.append(label_vec)
        ...
        total_pred_labels = tf.transpose(total_pred_labels, perm=[1, 0, 2]) # [batch_size, S_max, N_max]
        
        return generated_paragraph, pred_re, total_pred_labels
