...
os.chdir('/4t/jackyyeh/im2p/Krause_multilabel') # for batch inference
...
class ParagraphSolver(object):
    ...
    def validate(self, sess, sampled_paragraphs, pred_re, total_pred_labels, output_path, output_labels_path):
        ...
        for val_b in xrange(val_data.num_batch):
            ...
            _sampled_paragraphs, _pred, _total_pred_labels = sess.run([sampled_paragraphs, pred_re, total_pred_labels], feed_dict)
            ...
            batch_max_pred_words, batch_pred_scores = get_max_pred_words(_total_pred_labels, 20, self.data.idx2attrs)
            total_max_pred_labels.extend(batch_max_pred_words)
            total_pred_scores.extend(batch_pred_scores)

        output_paragraphs(totol_paragraphs, output_path)
        output_labels(total_max_pred_labels, total_pred_scores, output_labels_path) 
    ...
    def train(self):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            ...
            sampled_paragraphs, pred_re, total_pred_labels = self.model.build_sampler(reuse=True)
        ...
        tf_vars["total_pred_labels"] = total_pred_labels
        ...
        with open(os.path.join(self.log_path, self.log_file), 'w') as log:
            ...
            for e in range(self.n_epoch):
                ...
                # validate
                output_path = os.path.join( self.result_path, "val_candidate" + str(e+1) + ".txt" )
                output_labels_path = os.path.join( self.result_path, "val_pred_labels"+ str(e+1) +".txt")
                self.validate(sess, sampled_paragraphs, pred_re, total_pred_labels, output_path, output_labels_path)
    ...
    def inference(self):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            sampled_paragraphs, pred_re, total_pred_labels = self.model.build_sampler(reuse=False)
        ...
        if self.fixed_n_sent:
            ...
            output_labels_path = os.path.join( self.result_path, self.pretrained_model + "_pred_labels" + "_fixed_n_sent")
        else:
            output_path = os.path.join( self.result_path, self.pretrained_model)
            output_score_path = os.path.join( self.result_path, 'score')
            output_labels_path = os.path.join( self.result_path, self.pretrained_model + "_pred_labels")

        output_paragraphs(totol_paragraphs, output_path)
        output_labels(total_max_pred_labels, output_labels_path) 
